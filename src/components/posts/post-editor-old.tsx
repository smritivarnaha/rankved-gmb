...
// No fallback sample data ÔÇö profiles load from /api/profiles only
...
export function PostEditor({ initialData = null, timelineDate, onDateChange, lockedProfileId, returnUrl }: { initialData?: any; timelineDate?: string; onDateChange?: (d: string) => void; lockedProfileId?: string; returnUrl?: string }) {
  ...
  // Sync timeline date ÔåÆ schedule calendar
  useEffect(() => {
    if (timelineDate) {
  ...
  // Current time as HH:MM string (local) ÔÇö used as the min time when today is selected
  const nowTimeStr = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;

  // Next rounded-up 15-min slot from now (e.g. 13:47 ÔåÆ 14:00)
  ...
  const getScheduledAt = () => {
    ...
    // Build a Date from the local date/time strings ÔÇö this correctly applies
  ...
  const handleSave = async (type: string) => {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        // Partial success ÔÇö saved to DB but GBP publish failed
        alert(`ÔÜá´©Å Post saved but could not publish to Google:\n\n${responseData.error}\n\nCheck Settings to reconnect your Google account.`);
        router.push(returnUrl || (form.locationId ? `/profiles/${form.locationId}` : "/profiles"));
      } else {
        alert(responseData.error || "Failed to save post");
      }
    } catch (err) {
      alert("Network error ÔÇö could not save post.");
    }
    setSaving(false);
    setSavingType("");
  };

  // PUBLISHED posts are locked ÔÇö already sent to GBP, cannot be modified
  ...
  return (
    <>
      ...
      <div className="space-y-12">
        {/* ÔöÇÔöÇÔöÇ Top Selection Row ÔöÇÔöÇÔöÇ */}
              ...
              <div className="bg-white">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { if (calMonth === 0) { setCalMonth(11); setCalYear(y=>y-1); } else setCalMonth(m=>m-1); }} className="p-1 hover:bg-gray-100 rounded">ÔÇ╣</button>
                    <button onClick={() => { if (calMonth === 11) { setCalMonth(0); setCalYear(y=>y+1); } else setCalMonth(m=>m+1); }} className="p-1 hover:bg-gray-100 rounded">ÔÇ║</button>
                  </div>
                </div>
        ...
        {/* ÔöÇÔöÇÔöÇ Main Content Area ÔöÇÔöÇÔöÇ */}
              ...
              <div className="px-5 py-4 border-t border-[#f1f5f9] bg-white">
                {selectedDate && selectedTime ? (
                  ...
                  <div className="flex items-center gap-2 text-[#64748b]">
                    ...
                    <p className="text-[12px] font-medium italic">No schedule ÔÇö will save as draft</p>
                  </div>
                )}
              </div>
            </div>
