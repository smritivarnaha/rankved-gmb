...
function getStarCount(starRating: string | number): number {
...
export function ReviewManager({ profileId }: { profileId: string }) {
  ...
  const { data: profileData } = useSWR(`/api/profiles/${profileId}`, fetcher);
    ...
    try {
      const res = await fetch("/api/reviews/generate-reply", {
        ...
        body: JSON.stringify({
          ...
          reviewerName: review.reviewer?.displayName || "Valued Patient",
          rating: getStarCount(review.starRating),
        }),
      });
      const data = await res.json();
      setReplies(prev => ({ ...prev, [review.reviewId]: data.reply }));
    } catch (err) {
      console.error(err);
    } finally {
      setReplyingTo(null);
  ...
  const handleOpenTemplateModal = (review?: any) => {
    setTargetReviewForTemplate(review || null);
    setIsTemplateModalOpen(true);
  };

  const handleSelectTemplate = (renderedText: string) => {
    if (targetReviewForTemplate?.reviewId) {
      setReplies(prev => ({ ...prev, [targetReviewForTemplate.reviewId]: renderedText }));
    }
  ...
  const profileInfo = profileData?.data || profileData?.profile || {};
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="bg-slate-50 text-slate-700 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest border border-slate-200 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-4">
        {reviews.map((r: any) => {
          const starCount = getStarCount(r.starRating);
          return (
            <div key={r.reviewId} className="bg-white rounded-[24px] border-2 border-slate-100 p-6 hover:shadow-xl hover:shadow-slate-100/50 transition-all">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-black text-slate-900">{r.reviewer?.displayName || "Anonymous Patient"}</p>
                    <div className="flex items-center gap-1 mt-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < starCount ? "text-amber-400 fill-current" : "text-slate-200"}`} />
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-slate-50">
                  {!replies[r.reviewId] ? (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        onClick={() => handleOpenTemplateModal(r)}
                        className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-700 bg-slate-100 px-4 py-2 rounded-full hover:bg-slate-200 transition-all border border-slate-200"
                      >
                        <BookOpen className="w-3 h-3 text-indigo-600" />
                        Choose Template ({starCount}★)
                      </button>
                    </div>
                  ) : (
      ...
      {/* Dynamic Templates Library Modal */}
